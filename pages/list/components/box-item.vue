<template>
  ...
  <view v-show="i === index">
    ...
    <mescroll-body :ref="'mescrollRef'+i" :down="downOption" :up="upOption" top="100" bottom='100' @down="downCallback"
                   @init="mescrollInit" @up="upCallback">
      <!-- 数据列表 -->
      <view class="goods-box">
        <view v-for="(item,index) in goods" class="box-item" @click="jumpUrl(item)">
          ...
          <view class="good" v-if='item.state'>
            <u-image height="152rpx" bg-color='#212024' src="https://manghe.jikechaowan.com/image/box-3.png" width="180rpx"></u-image>
          </view>
          <view class="goods-empty" v-else>
            <u-image height="125rpx" bg-color='#212024' src="https://manghe.jikechaowan.com/image/box-2.png" width="150rpx"></u-image>
          </view>
...
export default {
  ...
  methods: {
    jumpUrl(item) {
      let url = '/pages/good/ordinary'
      if(this.jifen_id) item.jifen_id = this.jifen_id
      if (item.blind_type === 1) { //无限盲盒
...
.goods-box {
  ...
  .box-item {
    position: relative;
    background-image: url('https://manghe.jikechaowan.com/image/box-1.png');
